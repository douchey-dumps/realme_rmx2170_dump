...
if [ "$fw_path" != "" ]; then
	if [ -f $fw_path ];then
		dd if="${fw_path}" of=/cache/src_fw_header bs=1 count=40 skip=80;
		dd if=/dev/block/bootdevice/by-name/opporeserve1 of=/cache/local_fw_header bs=1 count=40 skip=0;
