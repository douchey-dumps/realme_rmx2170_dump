...
PHX_LOG_DIR="/mnt/vendor/opporeserve/media/log/hang_oppo"
...
TMP_DIR="/cache/phoenix_$(date +"%Y%m%d%H%M%S_%N")"
...
ERROR_FUNC_MAP_KEY=("ERROR_HANG_OPPO"
...
function check_opporeserve2_is_available()
{
    ...
    opporeserve2_remainsize=$(df -k | grep "${OPPORESERVE2_MOUNT_POINT}" | sed 's/[ ][ ]*/,/g' | cut -d "," -f4)
...
function phx_log_native_helper_main()
{
    ...
    do
        ...
        then
            ...
            then
                ...
                then
                    tar -czvf ${PHX_LOG_DIR}/${PHX_LOG_FILE_PATTERN}_$(date +%F-%H-%M-%S).tz  ${collect_tmp_dir}/*
                fi
            else
                rm -rf ${collect_tmp_dir}
                exit 0
    ...
    then
        ...
        then
            ...
            if [ "$OPPORESERVE2_PARTITION_AVAILABLE" == "1" ] ;
            then
                tar -czvf ${PHX_LOG_DIR}/${PHX_LOG_FILE_PATTERN}_$(date +%F-%H-%M-%S).tz ${RECOVERY_INFO_FILE} ${collect_tmp_dir}/*
            fi
